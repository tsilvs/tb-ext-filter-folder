...
const ACTION_IDS = [
	"btnGenMissingInbox", "btnGenMissingLeaf", "btnGenMismatched",
	"btnDeleteEmptyFolders", "btnDeleteInvalidRules",
	"btnCreateMissing", "btnCreateMissingInline",
	"btnCreateDiscovered", "btnGenRules",
	"btnDownloadRules", "btnSortInput", "btnApplyDefaults", "btnInfer",
];
// Only saved once at the start of any processing run; cleared on idle.
const _actionDisabledCache = new Map();

function renderProcessing(processing) {
	const isFolders = !!processing.folders;
	const isDiscovery = !!processing.discovery;
	const isAny = isFolders || isDiscovery;

	const analysisSection = $("analysisResults");
	if (analysisSection) analysisSection.classList.toggle("is-processing", isFolders);
	const discoverySection = $("contentDiscovery");
	if (discoverySection) discoverySection.classList.toggle("is-processing", isDiscovery);
	const btnAnalyze = $("btnAnalyze");
	if (btnAnalyze) btnAnalyze.disabled = isAny;
	const btnScan = $("btnScan");
	if (btnScan) btnScan.disabled = isAny;

	if (isAny) {
		ACTION_IDS.forEach((id) => {
			const btn = $(id);
			if (!btn) return;
			if (!_actionDisabledCache.has(id)) _actionDisabledCache.set(id, btn.disabled);
			btn.disabled = true;
		});
	} else {
		ACTION_IDS.forEach((id) => {
			const btn = $(id);
			if (!btn) return;
			if (_actionDisabledCache.has(id)) {
				btn.disabled = _actionDisabledCache.get(id);
				_actionDisabledCache.delete(id);
			}
		});
	}
}
...
function renderConfig(config) {
	...
	const btnScan = $("btnScan");
	if (btnScan)
		btnScan.textContent = browserApi.i18n.getMessage("scanAndDiscoverLimit", [
			config.scanLimit,
		]);
...
const updateRulesDirtyState = () => {
	...
	const isDirty =
		currentRules && currentRules.value !== store.getState().rulesSnapshot;
	if (currentRules) {
...
const appendGeneratedRules = (generated) => {
	...
	const deduped = RuleEngine.dedupeRawRules(combined);
	const dedupedByPath = RuleEngine.dedupeRawRulesByPath(deduped);
...
const withButtonBusy = async (button, action) => {
	...
	const siblings = block
		? Array.from(block.querySelectorAll("aside.stat button"))
		: [button];
...
function renderAnalysis() {
	...
	if (report && reportPanel) {
		...
		lines.push("");
		lines.push(
			`Folders that don't match expected rule path: ${mismatchedFolders.length}`,
		);
		mismatchedFolders.forEach((item) => {
			lines.push(
				`- ${item.email} actual: ${item.actualPath || item.rulePath} expected: ${item.expectedPath}`,
			);
		});
		lines.push("");
		lines.push("");
		lines.push(
			`Empty leaf folders (no senders found): ${emptyLeafFolders.length}`,
		);
	...
	if ($("missingInboxDetails"))
		$("missingInboxDetails").classList.toggle(
			"hidden",
			missingInboxRules.length === 0,
		);
	renderAnalysisList("listMissingInbox", missingInboxRules, (email) => email);

	if ($("missingLeafDetails"))
		$("missingLeafDetails").classList.toggle(
			"hidden",
			missingLeafRules.length === 0,
		);
	renderAnalysisList(
		"listMissingLeafRules",
		missingLeafRules,
		(item) => `${item.email} -> ${item.expectedPath}`,
	);

	if ($("mismatchedDetails"))
		$("mismatchedDetails").classList.toggle(
			"hidden",
			mismatchedFolders.length === 0,
		);
	renderAnalysisList(
		"listMismatchedFolders",
		mismatchedFolders,
		(item) =>
			`${item.email} actual: ${item.actualPath || item.rulePath} expected: ${item.expectedPath}`,
	);

	if ($("emptyFoldersDetails"))
		$("emptyFoldersDetails").classList.toggle(
			"hidden",
			emptyLeafFolders.length === 0,
		);
	renderAnalysisList("listEmptyLeafFolders", emptyLeafFolders, (path) => path);

	if ($("invalidRulesDetails"))
		$("invalidRulesDetails").classList.toggle(
			"hidden",
	...
	renderAnalysisList(
		...
		invalidRules,
		(item) =>
			`${item.email} -> ${item.rulePath} (expected ${item.expectedPath})`,
	);

	if ($("btnGenMissingInbox"))
		$("btnGenMissingInbox").disabled = missingInboxRules.length === 0;
	if ($("btnGenMissingLeaf"))
		$("btnGenMissingLeaf").disabled = missingLeafRules.length === 0;
	if ($("btnGenMismatched"))
		$("btnGenMismatched").disabled = mismatchedFolders.length === 0;
	if ($("btnDeleteEmptyFolders"))
		$("btnDeleteEmptyFolders").disabled = emptyLeafFolders.length === 0;
	if ($("btnDeleteInvalidRules"))
		$("btnDeleteInvalidRules").disabled = invalidRules.length === 0;
}
...
function renderLeafRulesMissing() {
	...
	const missing = store.getState().analysisMeta?.leafRulesMissing || [];
...
function renderInboxDebug() {
	...
	const meta = store.getState().analysisMeta?.inbox;
	if (!meta) {
	...
	lines.push(
		`Source Folder: ${meta.sourceFolder?.name || "Unknown"} (${meta.sourceFolder?.path || "n/a"})`,
	);
	lines.push(`Scan Limit: ${meta.scanLimit}`);
	lines.push(`Rule Emails: ${meta.ruleEmailCount}`);
	lines.push(
		`Inbox Senders: ${meta.totalSenders} (${meta.uniqueSenders} unique, ${meta.duplicateSenders} duplicates)`,
	);
	lines.push(
		`Missing Rules (Inbox): ${meta.missingCount} (${meta.missingUniqueCount} unique)`,
	);
	...
	lines.push("");
	lines.push(
		`Missing Inbox Emails (unique, showing ${Math.min(meta.missingEmails.length, 200)} of ${meta.missingEmails.length}):`,
	);
	meta.missingEmails.slice(0, 200).forEach((email) => lines.push(`- ${email}`));
	lines.push("");
	lines.push(
		`Mismatched Inbox Rules (showing ${Math.min(meta.mismatchedRules.length, 200)} of ${meta.mismatchedRules.length}):`,
	);
	meta.mismatchedRules
		.slice(0, 200)
		.forEach((item) =>
			lines.push(
				`- ${item.email}: rule=${item.rulePath} expected=${item.expectedPath}`,
			),
		);
	lines.push("");
	lines.push(
		`Invalid Inbox Emails (showing ${Math.min(meta.invalidEmails.length, 200)} of ${meta.invalidEmails.length}):`,
	);
	meta.invalidEmails.slice(0, 200).forEach((email) => lines.push(`- ${email}`));
	lines.push("");
	lines.push(
		`Inbox Emails (unique, showing ${Math.min(meta.inboxEmails.length, 200)} of ${meta.inboxEmails.length}):`,
	);
...
function renderDeleteEmptyReport() {
	...
	const meta = store.getState().analysisMeta?.deleteEmpty;
	if (!meta) {
		report.textContent = "";
		panel.classList.add("hidden");
		return;
	...
	lines.push("");
	lines.push(
		`Failures (showing ${Math.min(meta.failures.length, 200)} of ${meta.failures.length}):`,
	);
...
function renderMissingList() {
	const missing = store.getState().missing || [];
	const currentRoot = getCurrentRoot();
	const scopedMissing = currentRoot
		? missing.filter(
				(path) => path === currentRoot || path.startsWith(`${currentRoot}/`),
			)
		: missing;
	...
	if (scopedMissing.length === 0) {
		...
		$("btnCreateMissing").textContent = browserApi.i18n.getMessage(
			"createFolders",
		...
		if (inlineBtn) {
			...
			inlineBtn.textContent = browserApi.i18n.getMessage("createFolders", [
				scopedMissing.length,
			]);
		}
...
const getFilterTypeMask = () =>
	RuleEngine.calculateType(store.getState().config.filters);
...
const resolveFolderSelection = (folders, preference) => {
	...
	let match = id
		? folders.find((folder) => String(folder.id) === String(id))
		: null;
	if (!match && cleanPath) {
		match = folders.find(
			(folder) => folder.cleanPath.toLowerCase() === cleanPath.toLowerCase(),
		);
	}
	return match || null;
};
...
const validateTargetRoot = (folders, rootValue) => {
	...
	const match = folders.find(
		(folder) => folder.cleanPath.toLowerCase() === normalized.toLowerCase(),
	);
	return match
		? { value: match.cleanPath, valid: true }
		: { value: "", valid: false };
};
...
const saveAccountPreferences = async () => {
	...
	const accountPreferences = {
		...(store.getState().config.accountPreferences || {}),
	};
...
const collectPathWarnings = (paths, options = {}) => {
	const existingPaths = store
		.getState()
		.folders.map((folder) => folder.cleanPath);
...
const showWarningsDialog = (warnings) =>
	new Promise((resolve) => {
...
function validateAccountRulesMatch() {
	const currentRules = $("currentRules");
	if (!currentRules || !currentRules.value || !store.getState().accountBaseUri)
		return null;

	const rulesBaseUri = RuleEngine.extractBaseUri(currentRules.value);

	// If rules have placeholder or no URI, no mismatch
	if (!rulesBaseUri || rulesBaseUri === PLACEHOLDER_URI) return null;

	// Compare base URIs (case-insensitive)
	const match =
		store.getState().accountBaseUri.toLowerCase() ===
		rulesBaseUri.toLowerCase();

	return match
		? null
		: {
				accountUri: store.getState().accountBaseUri,
...
async function loadAccount(id) {
	...
	try {
		...
		const accountBaseUri =
			currentAccount.identities && currentAccount.identities.length > 0
				? `imap://${currentAccount.identities[0].email}`
				: PLACEHOLDER_URI;
		store.setState(
			{ currentAccount, accountBaseUri },
			{ type: "account:load" },
		);
	} catch (e) {
		console.error("Failed to get account details", e);
		store.setState(
			{ accountBaseUri: PLACEHOLDER_URI },
			{ type: "account:error" },
		);
	}
	...
	if (sourceSelect) {
		...
		store.getState().folders.forEach((f) => {
			...
			if (f.type === "inbox") opt.selected = true;
	...
	const accountPrefs = store.getState().config.accountPreferences || {};
	const prefs = accountPrefs[String(id)] || null;
	if (prefs) {
		const resolvedSource = resolveFolderSelection(
			store.getState().folders,
			prefs.source,
		);
		...
		const resolvedTarget = resolveFolderSelection(
			store.getState().folders,
			prefs.target,
		);
		if (resolvedTarget && rootSelect) {
			rootSelect.value = resolvedTarget.cleanPath;
		} else if (store.getState().config.defaultRoot && rootSelect) {
			rootSelect.value = store.getState().config.defaultRoot;
		}
	} else if (store.getState().config.defaultRoot && rootSelect) {
		rootSelect.value = store.getState().config.defaultRoot;
	}
...
const refreshFolderStats = (folders = store.getState().folders) => {
	...
	if (mismatchEl) {
		...
		if (ruleEmails.size !== stats.leafs && (ruleEmails.size > 0 || stats.leafs > 0)) {
...
function renderDiscovery() {
	...
	const sorted = [...store.getState().discovered].sort((a, b) => {
		const va = a[store.getState().sort.col] || "",
			vb = b[store.getState().sort.col] || "";
		return va.localeCompare(vb) * store.getState().sort.dir;
	});
	...
	if (btnCreate) {
		...
		btnCreate.textContent = browserApi.i18n.getMessage("btnCreateFoldersOnly", [
			selected.length,
		]);
	}
	...
	if (selectAll)
		selectAll.checked =
			store.getState().discovered.length > 0 &&
			store.getState().discovered.every((i) => i.selected);
...
async function runCreate(paths, statusId, btn) {
	...
	const port = browserApi.runtime.connect({ name: PORT_NAMES.CREATE_FOLDERS });
	const accountId = $("account").value;

	return new Promise((resolve) => {
		port.onMessage.addListener((msg) => {
			if (msg.type === MESSAGE_TYPES.PROGRESS) {
				setStatus(
					statusId,
					`${msg.current}/${msg.total}: ${msg.path}`,
					"progress",
				);
			} else if (msg.type === MESSAGE_TYPES.COMPLETE) {
				setStatus(
					statusId,
					browserApi.i18n.getMessage("doneStatus", [
						msg.results.created.length,
				...
				btn.disabled = false;
				resolve();
			} else if (msg.type === MESSAGE_TYPES.ERROR) {
		...
		port.postMessage({
			...
			accountId,
			paths,
			preferredRoot: store.getState().config.folderRoot,
		});
	});
}
...
async function runAnalysis(accountId, filterContent, options) {
	const { mergeCase, rootPath, folders, scanLimit, sourceFolder } = options;
	...
	const scopedMissing = rootPath
		? res.missing.filter(
				(path) => path === rootPath || path.startsWith(`${rootPath}/`),
			)
		: res.missing;

	const stats = MailClient.computeFolderStats(folders, rootPath);

	const leafEmailMap = new Map();
	const emptyFolders = [];
	for (const leafPath of stats.leafPaths) {
		const folder = folders.find((item) => item.cleanPath === leafPath);
		if (!folder) continue;
		try {
	...
	const parsedRules = RuleEngine.parse(filterContent || "");
	const ruleEmails = new Set(parsedRules.flatMap((rule) => rule.emails));
	const ruleEmailMap = RuleEngine.buildEmailToRulePathMap(parsedRules);
	const rulePaths = RuleEngine.getUniquePaths(parsedRules);
	const rootPrefix = rootPath ? `${rootPath}/` : "";
	const scopedRulePaths = rootPath
		? rulePaths.filter(
				(path) => path === rootPath || path.startsWith(rootPrefix),
			)
		: rulePaths;
	const rulePathSet = new Set(
		scopedRulePaths.map((path) => (mergeCase ? path.toLowerCase() : path)),
	);
	const leafRulesMissing = stats.leafPaths.filter(
		(path) => !rulePathSet.has(mergeCase ? path.toLowerCase() : path),
	);
	const missingInboxRules = inboxEmails.filter(
		(email) => !ruleEmails.has(email),
	);
	...
	const sourceMeta = sourceFolder
		? {
				...
				path: sourceFolder.cleanPath || sourceFolder.path || "",
			}
		: null;

	const analysis = RuleEngine.analyzeRulesAndFolders(
		rootPath,
		leafEmailMap,
		parsedRules,
	);
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	document
		...
		.forEach(
			(el) => (el.textContent = browserApi.i18n.getMessage(el.dataset.i18n)),
		);
	...
	let accounts = [];
	try {
		accounts = (await browserApi.accounts.list()).filter(
			(a) => a.type === "imap",
		);
	} catch (e) {
	...
	if (currentRules)
		currentRules.oninput = (e) => {
			syncRuleState(e.target.value);
		};
	if ($("targetRoot")) {
		$("targetRoot").onchange = () => {
			const validated = validateTargetRoot(
				store.getState().folders,
				getCurrentRoot(),
			);
			if (!validated.valid) {
			...
			try {
				await withButtonBusy(btnCreateMissingInline, async () => {
					...
					if (
						...
						setStatus("statusCreateMissingInline", browserApi.i18n.getMessage("noFoldersToCreate"), "info");
						return;
					}
					await runCreate(
						store.getState().missing,
						"statusCreateMissingInline",
		...
		btnDeleteInvalidRules.onclick = async () => {
			...
			try {
				await withButtonBusy(btnDeleteInvalidRules, async () => {
					...
					const currentRules = $("currentRules");
					if (currentRules) {
						// Filter at raw-block level to preserve each rule's original type/attributes.
						const content = currentRules.value;
						const marker = 'name="';
						const firstIdx = content.indexOf(marker);
						if (firstIdx !== -1) {
							const header = content.substring(0, firstIdx);
							const blocks = content.substring(firstIdx)
								.split(/(?=name=")/g)
								.map((block) => block.trim())
								.filter(Boolean)
								.filter((block) => {
									const emails = RuleEngine.extractEmailsFromBlock(block);
									return !emails.some((email) => invalidEmails.has(email));
								});
							currentRules.value = header + blocks.join("\n");
						} else {
							currentRules.value = "";
						}
						syncRuleState(currentRules.value);
						setStatus("statusDeleteInvalidRules", `Removed ${invalidEmails.size} invalid rule(s).`, "success");
					}
					await $("btnAnalyze")?.click();
				});
			} finally {
